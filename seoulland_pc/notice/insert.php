...
<?
    ...
    @extract($_GET);
    @extract($_SESSION);
    //새글삽입  $table , 폼입력값($html_ok, $subject, $content, $upfile[] (배열)) 세션변수가 넘어옴
   // upfile[0] = a1.jpg 파일명.확장자 upfile[1] = 파일명.확장자  upfile[2] = 파일명.확장자 
    ...
    // FILES["업로드된 파일"][""] => 첨부된 파일의 정보를 빼낼 수 있음
    // 자동으로 name(실제파일이름), tmp_name, type, size, error의 정보를 빼내어 배열에 담음
    // tmp는 임시파일 (복사-붙여넣기 하는 순간 복제파일을 만들고 그 복제파일을 복사함 / 원본 손상 방지)
    // size 용량
    // error true 혹은 false로 값이 빠짐
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		if (!$upfile_error[$i]) //에러가 발생되지 않으면
		{
			...
			if( $upfile_size[$i]  > 500000 ) {   // 기준은 1,024byte = 1kb
				echo("
				<script>
			...
			if (!move_uploaded_file($upfile_tmp_name[$i], $uploaded_file[$i]) )  // 업로드를 했는데 만약에 업로드 되지 않는 경우
                
            // move_uploaded_file(임시저장파일명,실제저장할 서버경로 파일명 )    => 파일 업로드를 시켜주는 함수!!
            //파일을 업로드하고 업로드 처리가 안되었을때 메시지
			{
				echo("
					<script>
	...
	if ($mode=="modify")  // 수정 모드
	{
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			if ($del_ok[$i] == "y")
			{
				...
				unlink($delete_path);    // 실제로 업로된 파일 삭제하는 명령어 unlink('삭제할 파일의 경로')
